...
export const useDashboardService = () => {
  const count = async (): Promise<CountDashboard> => {
    ...
    const response: AxiosResponse<CountDashboard> =
      await httpClient.get<CountDashboard>(url);
    return response.data;
  };

  const sum = async (): Promise<PurchasesFromMonth> => {
    const url = `${resourceURL}/sumpurchases?year=2021`;
    const response: AxiosResponse<PurchasesFromMonth> =
      await httpClient.get<PurchasesFromMonth>(url);
    return response.data;
  };
