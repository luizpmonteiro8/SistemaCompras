...
export const useMarketService = () => {
  const save = async (market: Market): Promise<AxiosRequestHeaders> => {
    const response: AxiosResponse<Market> = await httpClient.post<Market>(
      resourceURL,
      market,
    );
  ...
  const deleteMarket = async (id: number): Promise<void> => {
    const url = `${resourceURL}/${id}`;
    await httpClient.delete(url);
  };

  const loadMarket = async (id: number): Promise<Market> => {
    const url = `${resourceURL}/${id}`;
