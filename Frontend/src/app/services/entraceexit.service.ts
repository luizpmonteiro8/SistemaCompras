...
export const useEntraceExitService = () => {
  const save = async (entraceexit: EntraceExit): Promise<AxiosRequestHeaders> => {
    const response: AxiosResponse<EntraceExit> = await httpClient.post<EntraceExit>(resourceURL, entraceexit);
  ...
  const deleteEntraceExit = async (id): Promise<void> => {
    const url = `${resourceURL}/${id}`;
    await httpClient.delete(url);
  };

  const loadEntraceExit = async (id): Promise<EntraceExit> => {
    const url = `${resourceURL}/${id}`;
